//科目记账明细表
garen_define("js/report/finance/kmjzDetail",function (jqObj,loadParams) {
	...
	var querySubject = base.querySubject;//操作员查询
	...
	var exOpts = {
		...
		onBeforeExport:function(params){
			params.header = ["日期范围："+params["start_date"]+" 到  "+params["end_date"],"(单位:元)"];
			params.footer = ["出纳:","会计:",
			                 '负责人:'];
		},
		onLoad:function(){
			subjectTree = jqObj.findJq("subjectTree");
	...
	function loadTree(){
		$.postEx(querySubject,function(retJson){
			if(retJson.result && retJson.data){
				subjectTree.combotree('loadData',
					$.list2Tree(retJson.data,"subject_no","parent_subject_no","subject_name"));
			}
		});
	}
	...
	reportObj.parse(jqObj,'科目记账明细表',exOpts);
