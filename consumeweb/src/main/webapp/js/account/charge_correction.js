//充值纠错
garen_define("js/account/charge_correction",function (jqObj,loadParams) {
	...
	var chargeCorrectionDetailQuery = "account/chargeCorrectionDetailQuery.do";//充值明细查询
	
	var chargeCorrectionQuery = "account/chargeCorrectionQuery.do";//充值筛选查询
	...
	var toolbarWest = [null,{
		eName : 'div',
		cssClass : 'charge_correction_userinfo',
		elements : [{
			eName : 'div',
			cssClass : 'charge_correction_div',
			elements : [{
				eName : 'div',
				elements : [{
					eName : 'span',
					cssClass:"charge_correction_div_span",
					text : '学/工号',
					},{
						eName : 'textbox',
						name : 'user_no',
				...
				elements : {
					...
					width:60,
					height:30,
					cssClass:'charge_correction_chaxun',
					text:"查询",
					onClick : function(){
						searchWin(0);
	...
	var westUI = [{ //中上部分
		eName : 'div',
		cssClass : 'charge_correction_line',
		elements : {
			eName : 'span',
			text : '人员信息'
		}
...
//				}
			...
			onLoadSuccessEx:function(retJson){
				...
				charge_correction_beforemoeny.html("");
				charge_correction_aftermoney.html("");
			...
			onCheckEx:function(index,row){
				charge_correction_beforemoeny.html(row.cash_amt_str);
				if(charge_correction_beforemoeny.html()!="" && moneyBox.textbox("getValue")!=""){
					var ccbm = Math.round(Number(charge_correction_beforemoeny.html())*100); 
					var nv = Math.round(Number(moneyBox.textbox("getValue"))*100); 
					var v = ccbm - nv;
					var val = v/100 + "";
					
					if(val.indexOf(".") == -1){//输入时不含小数点（0.00==0）
						charge_correction_aftermoney.html(val+".00");
					}
					else if(val.length - val.indexOf(".") - 1 == 1){//小数点后面有一位数（0.10==0.1）
						charge_correction_aftermoney.html(val+"0");
					}
					else{//小数点后面有两位数
						charge_correction_aftermoney.html(val);
					}
				}
			}
		}
	...
	var centerUI = [{ //中上部分
			...
			eName : 'div',
			cssClass : 'charge_correction_center_bottom',
			elements : [{ //中下左
				eName : 'div',
				cssStyle:"width:340px;margin:0 auto;",
				elements:[{
					eName : 'div',
					cssClass : 'charge_correction_center_bottom_left_div1',
					elements : [{
						eName : 'span',
						cssClass : 'charge_correction_beforetext',
						text : '纠错前余额:',
						cssStyle : ''
						},{
							eName : 'span',
							cssClass : 'charge_correction_beforemoeny',
							cssStyle : '',
							text: ''
						},
						{
					...
					eName : 'div',
					cssClass : 'charge_correction_after',
					elements : [{
						eName : 'span',
						text : '纠错后余额:',
						cssClass : 'charge_correction_aftertext'
						},{
							eName : 'span',
							cssClass : 'charge_correction_aftermoney',
							text: ''
						},
						{
							eName : 'span',
					...
					eName : 'div',
					cssClass : 'charge_correction_inputmoney',
					elements : [{
						eName : 'span',
						text : '输入纠错金额',
						},{
...
//				{
//					eName : 'div',
//					cssClass : 'charge_correction_input_button',
//					elements : {
//						eName : 'div',
//						elements : [{
//							eName : 'linkbutton',
...
//				},
				{
					eName : 'div',
					cssClass : 'charge_correction_other',
					elements : {
						eName : 'div',
						elements : [
//						            {
...
//						},
	...
	var eastUI = [{ //中上部分
		...
		elements : {
			...
			idField : 'id',
			id:"dataTableEast",
			//url : chargeCorrectionDetailQuery,
	...
	var southUI = [{
		...
		eName : 'div',
		cssClass:"charge_correction_south",
		elements : [{
			eName : 'linkbutton',
			uId:"tm1",
			text : "<span style='font-size:20px;'>读卡</span>",
			cssClass : 'charge_correction_linkbutton',
			width : 220,
			height : 60,
			onClick : readCard
		},{
			eName : 'linkbutton',
			uId:"tm2",
			id:"charge_correction_button",
			text : "<span style='font-size:20px;'>充值纠错</span>",
			cssClass : 'charge_correction_linkbutton',
			width : 220,
			height : 60,
			onClick:charge_correction
		}]
	}];
	...
	var charge_correction_form = jqObj.findJqUI("charge_correction_form");
	var charge_correction_beforemoeny = jqObj.findJq("charge_correction_beforemoeny");
	var charge_correction_aftermoney = jqObj.findJq("charge_correction_aftermoney");
	...
	var charge_correction_button = jqObj.findJq("charge_correction_button");
	var account_id = jqObj.findJq("account_id");
	var searchBtn = jqObj.findJq("searchBtn");
	var charge_correction_print = jqObj.findJq("charge_correction_print");
...
//	};
	...
	function loadInit(){
		...
		$.postEx(checkCoreIp,{'op_name':'充值纠错'},function(retJson){
			if(retJson.result){
		...
		dataTableEast.datagrid("loadEx",{url:chargeCorrectionDetailQuery});
		
		var charge_correction_print_check = $.cookie("charge_correction_print_check");
		if(charge_correction_print_check == 1){
			charge_correction_print.prop("checked",true);
			$.cookie('charge_correction_print_check', charge_correction_print_check, { expires: 7 });
			printItem(0);//检测是否安装控件
		}
		else if(charge_correction_print_check == 2){
			charge_correction_print.prop("checked",false);
			$.cookie('charge_correction_print_check', charge_correction_print_check, { expires: 7 });
		}
	}
	
	function searchWin(cx_type){
		dataTableWest.datagrid("loadEx",{url:chargeCorrectionQuery,'cx_type':cx_type});
	}
	
	//搜索窗口
//	function searchWin(){
//		var params = charge_correction_form.form2Json();
//		if(params.dep_serial=="" && params.user_id=="" 
//			&& params.user_lname=="" && params.user_no==""){
//			$.alert("请输入一个查询条件！");
//		}else{
//			var p = {};
//			var myform = jqObj.findJq("charge_correction_form");
//			if(myform.form('form2Json',p)){//true则表单验证通过
//				searchBtn.linkbutton("disable");
//				$.postEx(chargeCorrectionQuery,params,function(retJson){
//					if(retJson.result){
...
//							charge_correction_beforemoeny.html(retJson.data[0].cash_amt_str);
//							charge_correction_aftermoney.html("");//若先输入金额则需要清空
...
//									callback:success
//								},
//								url:"js/account/charge_correction_searchWin.js"
//							});
//							searchBtn.linkbutton("enable");
//						}
//					}else{
	...
	function moneyInput(newValue, oldValue){
		...
		if(money.test(newValue)){//金额验证
			if(newValue>21474836.47){
				...
				charge_correction_aftermoney.html("");//清空充值后余额
			}else{
				//金额输入框
				...
				//纠错后余额
				if(charge_correction_beforemoeny.html()!=""){
					var ccbm = Math.round(Number(charge_correction_beforemoeny.html())*100); 
...
//		}
		if(row==null){
		...
		else if(Number(charge_correction_aftermoney.html())<0){
			$.alert("纠错金额不能超过纠错前余额！");
		}
		else{
			charge_correction_button.linkbutton("disable");
			var params = {};
			params['account_id'] = row.account_id;
			params['rchCorrect_amt'] = moneyBox.textbox("getValue");
			...
			var myWin = $.createWin({
				...
				queryParams:{
					params:params,
					callback:chargeCorrectionSuccess
				},
				url:"js/account/charge_correction_syncWin.js"
			});
		}
	}
	
	function chargeCorrectionSuccess(param,i){
		if(i){
			if(charge_correction_print.prop("checked")){
				var params = {};
				params['account_id'] = param.account_id;
				params['trad_amt'] = param.rchCorrect_amt;
				params['trad_before_amt'] = param.cash_amt_before;
				params['event_lx'] = "23";//充值纠错
		...
		charge_correction_button.linkbutton("enable");
		//account_id.val("");
	}
	...
	function setCookie(){
		if(charge_correction_print.prop("checked")){//选中
			printItem(0);//检测是否安装控件
			$.cookie('charge_correction_print_check', 1, { expires: 7 }); 
		}else{//未选中
			$.cookie('charge_correction_print_check', 2, { expires: 7 }); 
		}
	}
