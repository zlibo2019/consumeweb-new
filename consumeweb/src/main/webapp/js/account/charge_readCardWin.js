//充值读卡提示窗
garen_define("js/account/charge_readCardWin",function (jqObj,loadParams) {
	...
	function readCard(){
		// add by LYh 20170608jmjlink.readCard
		jmjlink.rechargeCard(function(jt){
			if(jt){
				var param = {};
				...
				$.postEx(allReadCard,param,function(retJson){
					if(retJson.result && retJson.data.length>0){
						if(exit == 1){
							loadParams.callback(retJson,1,param);
							jqObj.window("close");
						}
						
					}else{
						if(exit == 1){
							loadParams.callback(retJson,0);
							jqObj.window("close");
						}
					}
				});
