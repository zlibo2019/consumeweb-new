//销户失败结果提示窗
garen_define("js/account/closeAccount_resultWin_2",function (jqObj,loadParams) {
		...
		elements:[{
			...
			onClick:function(){
				if(loadParams.params.data.length==0){
					...
					utils.printGrid("销户失败结果表",dataTable,loadParams.params.data);
				}
			}
		},{
			eName:"linkbutton",
			...
			onClick:function(){
				if(loadParams.params.data.length==0){
					...
					//导出文档
					utils.exportDocByData2("销户失败结果表",dataTable,loadParams.params.data);
				}
			}
		}]
	};
