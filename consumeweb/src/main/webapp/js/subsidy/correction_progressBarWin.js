//纠错进度条提示窗
garen_define("js/account/correction_progressBarWin",function (jqObj,loadParams) {
	
	var utils = garen_require("utils");
	
	var correctionCorrect = "subsidy/correctionCorrect.do";//纠错
	...
	function loadInit(){
        ...
        $.each(loadParams.params, function (i, row) {
    		//将对象放到数组中
    		ids.push(row.sub_slave_id);
        });
        if(ids.length>300){
			batch(1,ids);//分批
		}else{
			params['slave_ids'] = ids.join(',');//主键参数
			if(loadParams.configs.begin_date){
				params['begin_date'] = loadParams.configs.begin_date;
			}
			if(loadParams.configs.end_date){
				params['end_date'] = loadParams.configs.end_date;
			}
			if(loadParams.configs.sub_amt){
				params['sub_amt'] = loadParams.configs.sub_amt;
			}
			
	        bar.progressBar('start');
	        $.postEx(correctionCorrect,params,function(retJson){
...
//								}
							...
							url:"js/account/correction_resultWin_1.js"
						});
						jqObj.window("close");
					}else{
						var myWin = $.createWin({
...
//								}
							...
							url:"js/account/correction_resultWin_2.js"
						});
						jqObj.window("close");
					}
				}
