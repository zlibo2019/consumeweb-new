//新增时段
garen_define("js/highConsume/consumeRule_addRuleTime",function (jqObj,loadParams) {
	...
	loadInit();
	function loadInit(){
		begin_date.findJqUI().setDate(loadParams.params.begin_time);
		end_date.findJqUI().setDate(loadParams.params.end_time);
	}
	
	function save(){
		var begin = begin_date.findJqUI().getDate();
		var end = end_date.findJqUI().getDate();
		var mealTime = loadParams.params;
		if(end<=begin){
			//$.alert("结束时间不能小于开始时间！");
