//消费规则
garen_define("js/consume/consumeRule",function (jqObj,loadParams) {
	
	//webService
	
	var crowdQuery = "consume/consumeRule/crowdQuery.do";//人群查询
	
	var ruleListQuery = "consume/consumeRule/ruleListQuery.do";//规则查询
	
	var ruleListDelete = "consume/consumeRule/ruleListDelete.do";//删除规则
	
	var limitQuery = "consume/consumeRule/limitQuery.do";//限制查询
	
	var limitUpdate = "consume/consumeRule/limitUpdate.do";//限制修改
	...
	var crowd_columns = [//列字段定义
		...
		elements:[{
			...
			elements:{
				...
				onBeforeSave:function(params){
					//$.print(params);
					if(crowdTable.datagrid("getSelected")!=null){
						params['crowd_id'] = crowdTable.datagrid("getSelected").crowd_id;
					}
				...
				onSave:function(retJson){
					if(retJson.result){
						...
						loadLimit(crowdTable.datagrid("getSelected"));
					}else{
						$.alert(retJson.info);
					}
				},
   	...
   	var westUI = {
		...
		elements : [{
			eName:"div",
			cssClass:"consumeRule_west_first_div",
			elements:[{
				eName:"img",
	...
	function loadRuleList(row){
		dataTable.datagrid("loadEx",{url:ruleListQuery,'crowd_id':row.crowd_id});
	}
	...
	function addRuleList(){
		...
		if(row==null){
			...
			var myWin = $.createWin({
				...
				url:"js/consume/consumeRule_addRuleList.js"
			});
		}
	}
	...
	function modifyRuleList(){
		...
		var row2 = crowdTable.datagrid("getSelected");
		if(row==null){
			$.alert("请选择一个规则！");
		}else{
			var params = {};
			params['crowd_id'] = row2.crowd_id;
			...
			var myWin = $.createWin({
				...
				url:"js/consume/consumeRule_modifyRuleList.js"
			});
		}
	}
