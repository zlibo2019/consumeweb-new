//修改规则
garen_define("js/consume/defaultRule_modifyRuleList",function (jqObj,loadParams) {
	
	var ruleListUpdate = "consume/defaultRule/ruleListUpdate.do";//修改规则
				...
				elements:[{
					...
					elements:[{
						...
						onChange:function(newValue,oldValue){
							var reg = /[ @#\$%\^&\*！!<>\\\/]+/i;
							if(rule_name.textbox("getValue")=="" || rule_name.textbox("getValue")==oldValue+"规则" && newValue!="" && !reg.test(newValue)){
								rule_name.textbox("setValue",newValue+"规则");
							}
						}
					}]
				},{
	...
	function loadInit(){
		...
		$.postEx(ruleListQuery,loadParams.params,function(retJson){
			if(retJson.result){
				meal_name.textbox("setValue",retJson.data[0].meal_name);
