//按部门身份更换人群进度条提示窗
garen_define("js/consume/peopleSet_progressBarWin_1",function (jqObj,loadParams) {
	
	var changeRuleRelated = "consume/changeRuleRelated.do";//批量充值
	...
	function loadInit(){
		...
		params['dep_serial'] = p.dep_serial;
		params['identity_id'] = p.tt_order;
        $.postEx(changeRuleRelated,params,function(retJson){
			if(retJson.result){
				...
				if(retJson.data.length<1){//没有失败数据
					...
					var myWin = $.createWin({
						...
						url:"js/consume/peopleSet_resultWin_1.js"
					});
					jqObj.window("close");
				}
			}else{
