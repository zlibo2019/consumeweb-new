...
protected static Log log = LogFactory.getLog(SysAction.class);   
	...
	public ModelAndView queryCharge(JsonPage jp,String user_no,String dep_serial,
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping("/chargeDetailQuery")
	public ModelAndView queryChargeDetail(JsonPage jp,HttpSession session) {
		String gly_no = (String) session.getAttribute(USER_NAME);
		Map<String,Object> paramMap = createMap()
				.put("gly_no",gly_no)
				.build();
	 ...
	 * 充值操作
	 */
	@RequestMapping("/chargeOperation")
	public ModelAndView operateCharge(JsonPage jp,String account_id,
			String recharge_amt,String read_card_number,String cash_amt,
			String read_media_id,HttpSession session) {
		String gly_no = (String) session.getAttribute(USER_NAME);
		Map<String,Object> paramMap = createMap()
				.put("account_id",account_id)
				.put("recharge_amt",formatMoney2FEN(recharge_amt))
				.put("cash_amt",formatMoney2FEN(cash_amt))
				.put("read_card_number",read_card_number)
				.put("read_media_id",read_media_id)
				.put("gly_no",gly_no)
				.build();
		jp = commonDao.remoteCall(jp,"finweb/acc/recharge/recharge.do",paramMap);
		return Json(jp);
	}
}
